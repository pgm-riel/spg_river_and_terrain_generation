...
void SimpleScene::RenderTexturesMesh(Mesh* mesh, Shader* shader, glm::vec3 position, glm::vec3 scale, glm::vec3 rotation, Texture2D* texture)
{
	if (!mesh || !shader || !shader->program)
		return;
...
// Render mesh PBR workflow
void SimpleScene::RenderMeshPBR(Mesh* mesh, Shader* shader, glm::vec3 position, glm::vec3 scale, Texture2D* albedo, Texture2D* roughness, Texture2D* normal)
{
	if (!mesh || !shader || !shader->program)
		return;
...
void SimpleScene::RenderTexturesMesh(Mesh* mesh, Shader* shader, glm::vec3 position, glm::vec3 scale, Texture2D* texture, int cubeMapTextureID, bool reflect)
{
	if (!mesh || !shader || !shader->program)
		return;
